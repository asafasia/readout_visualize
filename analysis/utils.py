import numpy as np
# from .readout_dataset import ReadoutDataset
...
def two_state_discriminator(Ig, Qg, Ie, Qe, b_print=True, b_plot=True):
    ...
    if b_plot:
        ...
        ax2.plot(Ig_rotated, Qg_rotated, ".", alpha=0.1,
                 label="Ground", markersize=2)
        ax2.plot(Ie_rotated, Qe_rotated, ".", alpha=0.1,
                 label="Excited", markersize=2)
        ax2.axis("equal")
        ax2.set_xlabel("I")
        ax2.set_ylabel("Q")
...
def find_closest_keys(target_L: float, target_A: float, lengths, amplitudes):
    """
    Find the closest length and amplitude keys 
    in a nested dict where keys are strings.
    ...
    target_A : float
        Approximate amplitude value.
    dataset : dict
        Nested dict: dataset[length_str][amp_str] = ...

    Returns
    -------
    length_key : str
        Exact length key in the dataset.
    amplitude_key : str
        Exact amplitude key in the dataset.
    """

    # Convert length keys to float and find nearest
    idx_L = np.argmin(np.abs(lengths - target_L))
    length_key = str(lengths[idx_L])

    if target_L > lengths.max() or target_L < lengths.min():
        raise ValueError(
            f"Target length {target_L} is out of bounds "
            f"({lengths.min()}, {lengths.max()})"
        )

    # Convert amplitude keys inside chosen length
    idx_A = np.argmin(np.abs(amplitudes - target_A))
    amplitude_key = str(amplitudes[idx_A])

    if target_A > amplitudes.max() or target_A < amplitudes.min():
        raise ValueError(
            f"Target amplitude {target_A} is out of bounds "
            f"({amplitudes.min()}, {amplitudes.max()})"
        )
    return length_key, amplitude_key
